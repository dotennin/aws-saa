...
(function () {
  ...
  scriptEl.src = '//r.r10s.jp/com/rat/js/ral-1.5.0.js';
  ...
  function sendResourcePerfData() {
    ...
    var Rp = getCookie('Rp');
    if (!Rp || Rp.substr(0,3) > '015') return; // 0.5% cut
    ...
    if (res && res.length) {
      ...
      for (var i = 0; i < res.length && limit > 0; ++i) {
        var r = res[i];
        if (r.name.match(domainFilter)) {
          var sample = {};
        ...
        if (metrics.length === 50 || (metrics.length && i === res.length - 1)) {
          RAT.addCustomEvent({
            options: ['url','ua'],
            eventType: 'perf',
            pData: {
  ...
  if (parameters.acc === 1 && parameters.aid === 1) {
